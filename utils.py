...
def get_activities_by_professor(professor_id) -> str:
    activities = ActivityController.get_activities_by_professor_id(professor_id)

    if activities:
        message = "My activities:\n\n"
        for activity in activities:
            joined_count = len(activity.participants)
            professor = ProfessorController.get(activity.professor_id)

            message += f"ID: {activity.activity_id}, Professor: {professor.user_name} Date: {activity.date}, Time: {activity.time}," \
                       f" Place: {activity.place}, Event: {activity.event}, Joined_Count: {joined_count}\n\n"
            ...
            message = "No created activities yet."
    return message